...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${category} - Blog IA | Machine Stories`,
    description: `Articles sur ${category.toLowerCase()} : tutoriels, analyses et actualités en intelligence artificielle.`,
    alternates: {
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const data = categoryData[slug] || { icon: '📂', description: `Articles sur ${category.toLowerCase()}.`, color: 'from-cyan-500 to-blue-500' };
        ...
        <div className="relative z-10 container mx-auto px-4">
          ...
          <div className="mt-8 max-w-2xl">
            ...
            <h1 className="font-heading text-4xl md:text-5xl font-bold text-white mb-4">
              <span className={`bg-gradient-to-r ${data.color} bg-clip-text text-transparent`}>{category}</span>
            </h1>
      ...
      <section className="bg-slate-950 border-t border-white/5 py-12">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap gap-3">
            {categories.filter(c => slugify(c) !== slug).map((cat) => (
              <Link key={cat} href={`/categorie/${slugify(cat)}`}>
