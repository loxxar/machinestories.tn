...
let adminArticlesCache: Article[] | null = null;
...
export function getAdminArticles(): Article[] {
  if (adminArticlesCache) return adminArticlesCache;
...
export function saveArticle(article: Partial<Article> & { slug: string, fileSlug: string, body: { raw: string } }): void {
  const { slug, fileSlug, body, ...data } = article;
  ...
  const content = matter.stringify(body.raw, frontmatter);
  
  if (!fs.existsSync(contentDirectory)) {
    fs.mkdirSync(contentDirectory, { recursive: true });
  }
  
  fs.writeFileSync(path.join(contentDirectory, `${fileSlug}.mdx`), content, 'utf8');
  clearCache();
}

export function deleteArticle(fileSlug: string): boolean {
  const fullPath = path.join(contentDirectory, `${fileSlug}.mdx`);
